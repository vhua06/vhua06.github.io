...
//能力建構 thomas yang 202012/16
if ($('.listitem').length) {
    $(document).ready(function () {
        $('.listitem h3, .listitem p').on('click', function () {
...
function myHistory() {
  ...
  // console.log("canvasbg.width", this.canvasbg.width);
  // history
  if (0 < $('.history').length) {
    ...
    var linkDist = Math.min( this.canvas.width,  this.canvas.height) / 2.4,
    lineBorder = 2.5;
    ...
    function draw() {
      ...
      var ctxfr = this.canvas.getContext('2d');
      var ctxbg = this.canvasbg.getContext('2d');

      ctxfr.globalCompositeOperation = 'destination-over';
      ctxfr.clearRect(0, 0,  this.canvas.width,  this.canvas.height); // clear canvas
      ctxbg.globalCompositeOperation = 'destination-over';
      ctxbg.clearRect(0, 0,  this.canvas.width,  this.canvas.height); // clear canvas

      ctxfr.save();
      ctxfr.translate( this.canvas.width / 2,  this.canvas.height / 2);
      ctxbg.save();
      ctxbg.translate( this.canvas.width / 2,  this.canvas.height / 2);
      ...
      function renderPoints(ctx, arr) {
        for (var i = 0; i < arr.length; i++) {
            ...
            if (circle.ttl<0) {}
            var xEscape =  this.canvas.width / 2 + circle.radius,
            yEscape =  this.canvas.height / 2 + circle.radius;
